...
const TileProviderError = Cesium.TileProviderError;
    ...
    function requestTileGeometry(provider, x, y, level, layerToUse, request) {
        ...
        let promise =  provider.processor.scheduleTask({url:derivedResoure.url,headers:headers,methodName:'fetchArrayBuffer'});
        ...
        promise.then(function (result) {
            ...
            if (defined(provider._heightmapStructure)) {
                return deferred.resolve(createHeightmapTerrainData(provider, buffer, level, x, y));
            }
            return deferred.resolve(createQuantizedMeshTerrainData(provider, buffer, level, x, y, layerToUse));
        });
