...
const HouseDataSource = require('./HouseDataSource');
const Cache = require('../../utils/Cache');
     ...
     */
    class HouseTileServiceImageryProvider {
        constructor(viewer,options,shaders) {
            ...
            this.processor =   new Cesium.TaskProcessor('VectorDecode', Number.POSITIVE_INFINITY);
        }
        ...
        loadStyle(){
            ...
            resource.fetchText().then(function(styleStr){
                let promise =  this.processor.scheduleTask({init:true,styleStr:styleStr,tileSize:this._tileWidth,
        ...
        requestJsonData(deferred,imageryProvider, url,xyz){
            ...
            let styleLevel = this.viewer.camera.getLevel();
            // let labels = imageryProvider._tileMatrixLabels;
            // styleLevel = defined(labels) ? labels[styleLevel] : styleLevel.toString();
            let newUrl = url.url + '&_useStyleLevel='+styleLevel;
            let promise =  this.processor.scheduleTask({url:url.url,needDecode:this.options.needDecode,level:styleLevel,
                filterLayerId:this.options.filterLayerId,rectangle:rectangleObj});


            promise.then(function(deferred,xyz,level,results){
                if(results == true){
                    deferred.reject();
                    return;
                }
                deferred.reject();
                this.jsonPromiseResult(xyz,level,results)
            }.bind(this,deferred,xyz,level),function(error ){
                deferred.reject();
            });
        ...
        jsonPromiseResult(xyz,level,results){
            if(!results){
                return;
            }
            let x = xyz.x;
            ...
            let ds = this.cache.get(name);
            if(!ds){
                ds = new HouseDataSource(name,results,level,this.viewer,this.options,this.lightMap,this.houseMap);
        ...
        updateLightMap(){
            for(let key  in this.cache.map){
                let houseDataSource = this.cache.map[key];
                houseDataSource.setLightMap(this.lightMap);
            }
        }
    }
module.exports =HouseTileServiceImageryProvider;
