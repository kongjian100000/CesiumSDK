...
const ComputeCommand = Cesium.ComputeCommand;
const ContextLimits = Cesium.ContextLimits;
const MipmapHint = Cesium.MipmapHint;
...
const ReprojectWebMercatorFS = Cesium.ReprojectWebMercatorFS;
const ReprojectWebMercatorVS = Cesium.ReprojectWebMercatorVS;
const when = Cesium.when;
const Imagery = Cesium.Imagery;
const ImagerySplitDirection = Cesium.ImagerySplitDirection;
const ImageryState = Cesium.ImageryState;
const TileImagery = Cesium.TileImagery;
    ...
    ImageryLayer.prototype.getImageryFromCache = function(x, y, level, imageryRectangle) {
        var styleLevel = this._imageryProvider.viewer.scene.camera.getLevel();
        var cacheKey = getImageryCacheKey(x, y, level,styleLevel);
    ...
    ImageryLayer.prototype.removeImageryFromCache = function(imagery) {
        var styleLevel = this._imageryProvider.viewer.scene.camera.getLevel();
        var cacheKey = getImageryCacheKey(imagery.x, imagery.y, imagery.level,styleLevel);
        delete this._imageryCache[cacheKey];
    };

    function getImageryCacheKey(x, y, level,styleLevel) {
        return JSON.stringify([x, y, level,styleLevel]);
    }
